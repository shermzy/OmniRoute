...
/*
 * Staged copy of src/main/runtime-server.ts for Chat On Steroids 2.1.11,
 * pinned in CI to commit f51acbccdd734f524799ea92bb747be765fba1e4.
 * The ../../../../ imports make this artifact readable in OmniRoute; install-cos-source.mjs
 * rewrites them to the normal sibling imports when copying the file into a CoS checkout.
 */
...
function flattenMessages(body: Record<string, unknown>): string {
  const supportedFields = new Set(['model', 'messages', 'stream']);
  const unsupported = Object.keys(body).filter(key => !supportedFields.has(key)).sort();
  if (unsupported.length) {
    throw new HttpError(
      400,
      `Unsupported request field(s): ${unsupported.join(', ')}`,
...
async function handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (request.method === 'GET' && url.pathname === '/healthz') {
    try {
      ...
      return json(response, 200, { ok: true, service: 'chat-on-steroids-runtime', version: VERSION, ready: true });
    } catch (error) {
      return json(response, 503, { ok: true, service: 'chat-on-steroids-runtime', version: VERSION, ready: false,
